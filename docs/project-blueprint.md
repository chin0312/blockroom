...
- Browser Fullscreen API expands the existing workspace; unsupported mobile
  browsers use an in-page expanded mode without remounting the meeting stage.
- Member view uses a responsive one-to-six tile grid. Mobile shows two per
  portrait page or three per short landscape page; tablet uses four where
  needed. Page indices clamp when membership changes.
...
- Explicit leave, refresh, close or account change freezes the continuous
  visit. Switching among supported chains updates presence metadata without
  resetting the active Room timer; the eventual record remains attached to the
  chain on which that visit began.
...
- Raw Presence records are connection sessions. Canonical participants are
  grouped by lower-cased wallet address before counting, rendering or admission.
...
- Capacity is six unique normalized wallet addresses, never six tabs, chains,
  sockets or raw Presence records.
- Participant groups are ordered by their earliest active `joinedAt`, with the
  normalized address as a deterministic tie-break. Every session belonging to
  the first six groups is admitted; a second session for an existing wallet
  never consumes another slot.
- Supabase candidates settle before the local client becomes joined. A later
  over-capacity session is untracked/unsubscribed and receives a Room Full error.
  Canonical rendering is capped by the same admission selector, so an excess
  raw candidate never creates a seventh avatar.
- Same-browser fallback uses the Web Locks API for an atomic localStorage
  admission section where supported, then applies the same deterministic model.
- Supabase Presence does not provide an application-level atomic compare-and-set
  across clients. A simultaneous race is therefore resolved by deterministic
  reconciliation after Presence convergence, not by a falsely claimed backend
  lock. A trusted database RPC would be required for strict network-atomic admission.

### Canonical participant and connection sessions

- Canonical key: `<roomSlug>:<normalizedWalletAddress>`; chain ID is metadata.
- Session key: the unique `clientId` generated by each tab/device connection.
- A participant is created when at least one admitted session for its address is
  active and removed only when its final session leaves or expires.
- Intentional leave untracks that exact session immediately. Same-browser stale
  records expire after 90 seconds; abnormal Supabase disconnect cleanup follows
  the provider's Presence timeout.
- Display/media context uses the visible session with the newest meaningful
  `activityAt`; heartbeats change only `updatedAt`, preventing chain flicker.
  Stable client-ID ordering resolves ties.
- The earliest active session is `primarySessionId` for focus timing. Additional
  tabs for the same wallet do not start duplicate focus timers merely by joining.
...
type RoomParticipant = RoomMember & {
  participantKey: string; // normalized wallet address
...
| `ActiveMembers.tsx` | Canonical wallet member grid, media players, pin stage and responsive pagination |
...
| `room-presence.ts` | Address normalization, session grouping, deterministic admission, capacity and pagination selectors |
| `supabase-realtime.ts` | Shared Auth-disabled Supabase Realtime client |

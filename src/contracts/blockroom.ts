...
export const BLOCKROOM_CHAIN_ID = monadTestnet.id;
...
export const blockRoomAbi = parseAbi([
...
const configuredAddress = process.env.NEXT_PUBLIC_BLOCKROOM_CONTRACT_ADDRESS;

export const blockRoomAddress: Address | undefined =
  configuredAddress && isAddress(configuredAddress) ? configuredAddress : undefined;
export const blockRoomDeploymentTx = process.env.NEXT_PUBLIC_BLOCKROOM_DEPLOYMENT_TX as Hex | undefined;
export const blockRoomExplorerUrl = monadTestnet.blockExplorers?.default.url;
...
export function createSessionId({
  ...
  return keccak256(
    stringToHex(`${BLOCKROOM_CHAIN_ID}:${walletAddress.toLowerCase()}:${roomSlug}:${nonce}`),
  );
}

export function transactionExplorerUrl(hash?: Hex) {
  return hash && blockRoomExplorerUrl ? `${blockRoomExplorerUrl}/tx/${hash}` : undefined;
}
